# Slightly modified code from Antoine Miech: https://github.com/antoine77340/S3D_HowTo100M
...
PRETRAINED_S3D_WEIGHTS = 'https://www.rocq.inria.fr/cluster-willow/amiech/howto100m/s3d_howto100m.pth'
PRETRAINED_S3D_DICT = 'https://www.rocq.inria.fr/cluster-willow/amiech/howto100m/s3d_dict.npy'
...
class Sentence_Embedding(nn.Module):
    def __init__(
        ...
        token_to_word = np.load(token_to_word_path, allow_pickle=True)
...
class S3D(nn.Module):
    def __init__(self, num_classes=512, gating=True, space_to_depth=True, pretrained=True):
        ...
        # Only tack on the FC and text module if loading the pretrained model
        if pretrained:
            self.fc = nn.Linear(self.mixed_5c.output_dim, num_classes)
            with tempfile.NamedTemporaryFile() as s3d_dict:
                r = requests.get(PRETRAINED_S3D_DICT, allow_redirects=True)
                s3d_dict.write(r.content)
                self.text_module = Sentence_Embedding(
                    num_classes, token_to_word_path=s3d_dict.name)

            with tempfile.NamedTemporaryFile() as s3d_weights:
                r = requests.get(PRETRAINED_S3D_WEIGHTS, allow_redirects=True)
                s3d_weights.write(r.content)
                self.load_state_dict(th.load(s3d_weights.name))
    ...
    def forward(self, inputs):
        ...
        assert inputs.dim() == 5, "Need [B, T, C, H, W] as inputs to S3Dg."
        inputs = inputs.permute(0, 2, 1, 3, 4)  # We want to use the above ordering, so hax.
